  ...
  definition(t) {
    ...
    t.nullable.field('sendChat', {
      ...
      resolve: async (_, { file, to, content, type }, { user, prisma }) => {
        ...
        const target = await prisma.chatSession.findUnique({
          where: { id: to },
        })
        ...
        for (const p of uniq(target.participantsIds)) {
          publish(
  ...
  definition(t) {
    t.field('chatSubscribe', {
      ...
      async subscribe(_, __, { user, ...rest }) {
        if (!user?.id)
          return pubsub.asyncIterator<Chat>(['PRIVATE_CHAT_NOLOGIN'])

        return pubsub.asyncIterator<Chat>(['PRIVATE_CHAT_CREATED_' + user.id])
      },
